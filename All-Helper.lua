        ...
        if response.status_code == 200 then
            downloadUrlToFile(decodeJson(response.text)['url'], thisScript().path, function (id, status, p1, p2)
                ...
                if status == dlstatus.STATUSEX_ENDDOWNLOAD then
                    sampAddChatMessage('[All Helper]: {ffffff}Скрипт был успешно обновлён. Сейчас он будет перезапущен.', 0x696969)
                    thisScript():reload()
                end
            end)
        else
...
local commandsList_second =
{
	{'/recname', 'перезаход с указанным ником'},
	{'/punish', 'большая часть правил в одном окне'},
	{'/aroll', 'автоматичиское открытие рулеток'},
	{'/spec', 'отображение нажатий клавиш игроков'}
}
...
local lastUpdate =
{
	{'{ffcc00}29.03.2025 {ffffff}— обновление. {ffcc00}Версия: 3.60'},
	{'{ffffff}• Была добавлена возможность {ffcc00}обновлять скрипт прямо в игре'}, 
	{'{ffffff}Автоматическое обновление отсутствует. Обновить скрипт можно самостоятельно нажав на кнопку "Обновить скрипт"'},
	{'• Исправлены некоторые ошибки'},
	{'• Добавлена авто-прокрутка рулеток'},
	{'• Добавлена возможность отслеживать нажатия клавиш игроков в зоне стрима и слежки'},
	{'• Элементы управления окнами были изменены'},
	{'Под замену попали кнопки закрытия окон. Они были заменены на новые, так же заменены кнопки в разделе "Проведение МП"'},
...
---
----------------- INICFG -----------------
local delltext = {'Ace_Will', '.* через сайт%: attractive%-rp.ru', 'SAN', 'SAN', 'сайте', 'Подсказка', '/vacancy', '.* отправил VIP пользователь %w+_%w+%[%d+%]'}
----------------- NAVIGATION LIST -----------------
local navigation = {
    current = 1,
...
local AllWindows = imgui.OnFrame(function() return menu[0] end, function()
	if menuSwitch == 0 then
		...
		local sizeX, sizeY = 660, 369
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY))
		imgui.PushFont(myFont)
		imgui.Begin(faicons(u8'SHIELD_CHECK')..u8' All Helper', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
		imgui.CenterText(faicons(u8'SHIELD_CHECK')..u8' All Helper')
		imgui.SameLine(0,259)
		ad.CloseButton('MainClose', menu, 25)
		imgui.BeginChild('##menu', imgui.ImVec2(325,208), 1)
		...
		if imgui.Button(faicons(u8'CIRCLE_INFO')..u8' Список изменений      ', imgui.ImVec2(150, 35)) then
			menu[0] = false
			printStyledString(cyrillic("Последнее обновление вышло ~g~29.03.2025"), 1500, 5)
			sampShowDialog(1914, 'Список изменений', updateText, 'Закрыть', '', 0)
		end
		if imgui.Button(faicons(u8'ARROW_DOWN_TO_LINE')..u8'  Обновить скрипт', imgui.ImVec2(308, 35)) then
			update():download()
		end
		imgui.EndChild()
		imgui.SameLine(335)
		imgui.BeginChild('##menuDes', imgui.ImVec2(317,208), 1)
		...
		imgui.Separator()
		imgui.BeginChild('##menuDes2', imgui.ImVec2(310,85), 0)
		for i=1, #commandsList_first do
			imgui.Text(u8(commandsList_first[i][1] .. " — " .. commandsList_first[i][2]))
		end
		imgui.EndChild()
		imgui.SameLine()
		imgui.BeginChild('##menuDes3', imgui.ImVec2(310,85), 0)
		for i=1, #commandsList_second do
			imgui.Text(u8(commandsList_second[i][1] .. " — " .. commandsList_second[i][2]))
		end
		imgui.EndChild()
		imgui.PopFont()
		imgui.End()
	elseif menuSwitch == 1 then
		local resX, resY = getScreenResolution()
		local sizeX, sizeY = 925, 360
		imgui.PushFont(myFont)
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY))
		imgui.Begin(u8'Проведение отбора', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
		imgui.CenterText(u8'Проведение отбора')
		...
		imgui.EndChild()
		imgui.SetCursorPos(imgui.ImVec2(155, 35))
	...
	elseif menuSwitch == 2 then
		local resX, resY = getScreenResolution()
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(485, 265))
		imgui.PushFont(myFont)
		imgui.Begin(u8'Проведение МП', menu, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
		imgui.CenterText(u8'Проведение МП')
		imgui.SameLine(0,165)
		ad.CloseButton('MainClose', menu, 25)
		if mp_navigation.current == 1 then -- Home
			if imgui.BeginChild('child##1', imgui.ImVec2(470,215), 0) then
				...
				imgui.Text('')
				-- imgui.SameLine(100)
				if ad.MaterialButton(faicons('play')..u8'  Начать', imgui.ImVec2(150,25)) then
				...
				imgui.SameLine()
				if ad.MaterialButton(faicons('pause')..u8'  Закончить', imgui.ImVec2(150,25)) then
					...
					if u8:decode(ffi.string(set_namemp)) ~= '' and u8:decode(ffi.string(set_priz)) ~= '' and u8:decode(ffi.string(input_name)) ~= '' and selected_prize ~= 0 then
						sampSendChat('/msg Победитель МП "'..u8:decode(ffi.string(set_namemp))..'" на '..u8:decode(ffi.string(set_priz))..' '..selected_prize..' — '..u8:decode(ffi.string(input_name))..'. Поздравляем!')
					else
						sampAddChatMessage('[Ошибка] {ffffff}Значения выбраны неверно', 0x696969)
					end
				end
				...
				imgui.SameLine()
				if ad.MaterialButton(faicons('star')..u8'  Выдать', imgui.ImVec2(150,25)) then
					local set_namemp = str(set_namemp)
	...
	elseif menuSwitch == 3 then
		local resX, resY = getScreenResolution()
		local sizeX, sizeY = 695, 245
		imgui.PushFont(myFont) 
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY), imgui.Cond.FirstUseEver)
		imgui.Begin(u8'Настройки', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
		imgui.CenterText(u8'Настройки')
		imgui.SameLine(0,285)
		ad.CloseButton('MainClose', menu, 25)
	...
	elseif menuSwitch == 4 then
		...
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY), imgui.Cond.FirstUseEver)
		imgui.Begin(u8'Выдача выговоров', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
		imgui.CenterText(u8'Выдача выговоров')
		imgui.SameLine(0,255)
		ad.CloseButton('MainClose', menu, 25)
		...
		elseif menuSwitch == 5 then
			local resX, resY = getScreenResolution()
			local sizeX, sizeY = 505, 210
			imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY))
			imgui.Begin(u8'Флудер', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
			imgui.CenterText(u8'Флудер')
			imgui.SameLine(0,195)
			ad.CloseButton('MainClose', menu, 25)
			for i = 1, 5 do
		...
		elseif menuSwitch == 6 then
			...
			imgui.SetNextWindowPos(imgui.ImVec2(resX / 2, resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.SetNextWindowSize(imgui.ImVec2(sizeX, sizeY), imgui.Cond.FirstUseEver)
			imgui.Begin(u8'Другое', menu, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar)
			imgui.CenterText(u8'Другое')
			imgui.SameLine(0,295)
			ad.CloseButton('MainClose', menu, 25)
			imgui.BeginChild('##otherSettings1', imgui.ImVec2(300,120), 0)
			...
			imgui.EndChild()
			imgui.SameLine()
			imgui.BeginChild('##settings2', imgui.ImVec2(350,120), 0)
...
imgui.OnFrame(function() return target ~= -1 end, function(self)
	self.HideCursor = true
	imgui.SetNextWindowPos(imgui.ImVec2(sW / 2, sH - 100), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
	...
	imgui.PushFont(myFont)
		if doesCharExist(target) then
			...
			if plState == "onfoot" then
				imgui.BeginGroup()
				KeyCap("Shift", (keys[plState]["Space"] ~= nil), imgui.ImVec2(75, 30)); imgui.SameLine()
				KeyCap("Alt", (keys[plState]["Alt"] ~= nil), imgui.ImVec2(55, 30))
				KeyCap("Space", (keys[plState]["Shift"] ~= nil), imgui.ImVec2(135, 30))
				imgui.EndGroup()
				imgui.SameLine()
				imgui.BeginGroup()
					KeyCap("C", (keys[plState]["C"] ~= nil), imgui.ImVec2(30, 30)); imgui.SameLine()
					KeyCap("F", (keys[plState]["F"] ~= nil), imgui.ImVec2(30, 30))
					KeyCap("LM", (keys[plState]["RKM"] ~= nil), imgui.ImVec2(30, 30)); imgui.SameLine()
					KeyCap("RM", (keys[plState]["LKM"] ~= nil), imgui.ImVec2(30, 30))		
				imgui.EndGroup()
			else
				imgui.BeginGroup()
...
function event.onShowDialog(dialogId, style, title, button1, button2, text)
	if workInfo['working'] then
		if text:find('Обмен на золотые монеты и другие привилегии') ~= nil then sampSendDialogResponse(dialogId, 1, 4, '') end
		if title:find('Рулетки') ~= nil then 
...
function event.onServerMessage(color, text)
    ...
    -- end
	if workInfo['working'] then
		local roulletes = text:match('Вы прокрутили %W+ рулетку | Осталось (%d+) шт.')
		if roulletes ~= nil then
			if workInfo['roulletes'] < 1 then
				workInfo['working'] = false
				workInfo['roullete'] = 0
				workInfo['roulletes'] = 0
				sampAddChatMessage('Прокручены все рулетки! (Прокручено указанное количество)', -1)
			else
				if tonumber(roulletes) == 0 then
					workInfo['working'] = false
					workInfo['roullete'] = 0
					workInfo['roulletes'] = 0
					sampAddChatMessage('Прокручены все рулетки! (Больше не осталось)', -1)
				else
					workInfo['roulletes'] = workInfo['roulletes']-1
					sampSendClickTextdraw(gifttd[workInfo['roullete']])
				end
			end
		end
	end
...
----------------- ONSENDCOMMAND -----------------
event.onShowTextDraw = function(textdrawId, textdraw)
	local reid = -1
    if textdraw.text:find('~w~%w+_%w+ %(%d+%)~n~') then
        reid = textdraw.text:match('~w~%w+_%w+ %((%d+)%)~n~')
    elseif textdraw.text:find('~w~%w+ %(%d+%)~n~') then
        reid = textdraw.text:match('~w~%w+ %((%d+)%)~n~')
    end
    if reid ~= -1 then
        reid = tonumber(reid)
        local pedExist, ped = sampGetCharHandleBySampPlayerId(reid)
        if pedExist then
            target = ped
        end
    end
-- 
	if textdraw.text == 'Gift#1' then gifttd[1] = textdrawId
	elseif textdraw.text == 'Gift#2' then gifttd[2] = textdrawId
	elseif textdraw.text == 'Gift#3' then gifttd[3] = textdrawId end
	if workInfo['working'] and textdrawId == gifttd[workInfo['roullete']] then
		if gifttd[workInfo['roullete']] > 0 then
			workInfo['roulletes'] = workInfo['roulletes']-1
			sampSendClickTextdraw(gifttd[workInfo['roullete']])
		end
	end
...
function event.onTogglePlayerSpectating(state)
    if not state then
        target = -1
    end
end
---
event.onTogglePlayerControllable = function(controllable)
	if not controllable and workInfo['working'] then return false end
end
event.onDisplayGameText = function(style, time, text)
	if workInfo['working'] and style == 4 and (text:find('~b~Rolling Roulette') or text:find('~p~%+%d+ Gifts') or text:find('F-COINS') or text:find('~w~%+%d+ EXP') or text:find('~y~%+%d+ RUB')) then return false end
end

function astartRolling(arg)
	if workInfo['working'] then
		workInfo['working'] = false
		workInfo['roullete'] = 0
		workInfo['roulletes'] = 0
		sampAddChatMessage('{696969}[All Helper]: {ffffff}Прокрутка рулеток была остановлена принудительно', -1)
		return
	end
	local errorString = 
	{
		'{696969}[All Helper]: {ffffff}Используйте: /aroll [тип] [количество (от 1 до 1000)]',
		'{696969}[All Helper]: {ffffff}Типы рулеток: 1 - бронзовые рулетки, 2 - серебряные рулетки, 3 - золотые рулетки'
	}
	local args = split(arg, ' ')
	if type(tonumber(args[1])) ~= 'number' or tonumber(args[1]) < 1 or tonumber(args[1]) > 3 then
		sampAddChatMessage(errorString[1], 0xffffff)
		sampAddChatMessage(errorString[2], 0xffffff)
	else
		if type(tonumber(args[2])) ~= 'number' or tonumber(args[2]) < 1 or tonumber(args[2]) > 1000 then
			sampAddChatMessage(errorString[1], 0xffffff)
			sampAddChatMessage(errorString[2], 0xffffff)
		else
			workInfo['working'] = true
			workInfo['roullete'] = tonumber(args[1])
			workInfo['roulletes'] = tonumber(args[2])
			sampAddChatMessage('Прокрутка рулеток начата! {ff0000}Не вводите /donat!', -1)
			sampAddChatMessage('Чтобы остановить используйте: /aroll', -1)
			sampSendChat('/donat')
		end
	end
end
...
function main()
    ...
    if ipport ~= '62.122.213.231:7777' then
        ...
        sampAddChatMessage('[All Helper]: {ffffff}Ошибка: IP Attractive RP: {00aaff}Именной - a.attractive-rp.ru:7777 || Цифровой - 62.122.213.231:7777', c)
	...
	sampRegisterChatCommand('map', function() sampSendChat('/newobj') end)
	sampRegisterChatCommand('nl', function() sampSendChat('/nolimits') end)
	sampRegisterChatCommand('aroll', astartRolling)
	sampRegisterChatCommand("spec", function(playerId)
		if playerId == "off" then
			...
			printStringNow(cyrillic("~w~Используйте: ~y~/spec [ID]~n~~w~или ~y~/spec off~w~ чтобы выключить"), 3000)
		end
	end)
    while true do 
        wait(0)
...
function event.onVehicleSync(playerId, vehicleId, data)
	local result, id = sampGetPlayerIdByCharHandle(target)
	if result and id == playerId then
		keys["vehicle"] = {}
